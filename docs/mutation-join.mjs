...
for (const [file, info] of Object.entries(report.files)) {
  for (const m of info.mutants) {
    ...
    const coveredFiles = new Set(
      (m.coveredBy ?? []).map((t) => testFileById.get(t)).filter(Boolean),
    );
    ...
    if (m.status === 'Killed') killedTotal++;
      ...
      if (coveredFiles.size) survivedCovered++;
    } else if (m.status === 'NoCoverage') noCoverage++;
    const srcFile = rel(file);
    for (const f of coveredFiles) {
      get(f).covered.add(id);
      if (siblingOf(f) === srcFile || siblingOf(f).replace(/\.tsx$/, '.ts') === srcFile)
        get(f).ownCovered.add(id);
    }
    for (const f of killerFiles) {
      get(f).killed.add(id);
      if (siblingOf(f) === srcFile || siblingOf(f).replace(/\.tsx$/, '.ts') === srcFile)
        get(f).ownKilled.add(id);
    }
    if (killerFiles.size === 1) get([...killerFiles][0]).unique.add(id);
  }
}
console.log(
  `mutants ${totalMutants} · killed ${killedTotal} · survived-but-covered ${survivedCovered} · no coverage ${noCoverage}`,
);
const rows = [];
for (const [file, s] of per) {
  ...
  rows.push({
    ...
    killRate: covered ? killed / covered : null,
    ownCovered,
    ownKilled,
...
console.log(
  `Spearman(uselessness score, own-source survival): ${rho(
    own.map((r) => r.score),
    own.map((r) => 1 - r.ownRate),
  ).toFixed(2)}`,
);
for (const c of ['tautology', 'weak', 'mockBurden', 'cost', 'mirror'])
  console.log(
    `Spearman(${c}, own-source survival): ${rho(
      own.map((r) => r.components[c]),
      own.map((r) => 1 - r.ownRate),
    ).toFixed(2)}`,
  );
...
console.log(
  `Spearman(uselessness score, survival rate = 1 - kill%): ${rho(
    usable.map((r) => r.score),
    usable.map((r) => 1 - r.killRate),
  ).toFixed(2)}`,
);
console.log(
  `Spearman(weak share, survival rate): ${rho(
    usable.map((r) => r.weak),
    usable.map((r) => 1 - r.killRate),
  ).toFixed(2)}`,
);
for (const c of ['tautology', 'weak', 'mockBurden', 'cost', 'mirror'])
  console.log(
    `Spearman(${c}, survival): ${rho(
      usable.map((r) => r.components[c]),
      usable.map((r) => 1 - r.killRate),
    ).toFixed(2)}`,
  );
const byVerdict = {};
